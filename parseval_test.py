...
def get_data(filename, variablename, hfil=None):
    if Path(filename).is_file():
        try: 
            ...
            print("Not decoding time coordinate.")
    ...
    #
    #  this block is for the OLR test data, which has a bad time coordinate.
    #     
    if ds['time'].units == 'days since 0-0-0 00:00:0.0':
        time = ds['time']
        time.attrs['units'] = 'days since 0001-01-01 00:00:0.0'
        time.attrs['calendar'] = '365_day'
...
if __name__ == "__main__":
    ...
    data_variance = data.sel(lat=slice(-15,15)).weighted(data.time.dt.daysinmonth).var()
    print(f"Variance of data: {data_variance.item()}")
    #
    ...
    print(symComponent)
    print(f"SUM OF SYMMETRIC: {symComponent.sum().item()}")
    print(f"SUM OF ASYMMETRIC: {asymComponent.sum().item()}")
# Parseval's Theorem means that the sum of the spectral power density should equal the variance of the field.
